            ...
            <div className="container mx-auto max-w-7xl">
              <div className="mb-8">
                <div className="flex flex-wrap gap-2">
                  <Link to="/" className="text-slate-500 text-sm font-medium leading-normal">Home</Link>
                  <span className="text-slate-500 text-sm font-medium leading-normal">/</span>
                  <Link to="/products/all-sneakers" className="text-slate-500 text-sm font-medium leading-normal">Shoes</Link>
                  <span className="text-slate-500 text-sm font-medium leading-normal">/</span>
                  <span className="text-slate-800 text-sm font-medium leading-normal">{productTitle}</span>
                </div>
              </div>
              <div className="grid grid-cols-1 lg:grid-cols-2 gap-12 xl:gap-16">
                <div className="flex flex-col gap-4">
                  <div className="w-full aspect-square bg-white rounded-xl shadow-sm overflow-hidden">
                    ...
                    <div className="grid grid-cols-4 gap-4">
                        {product.images.map((img, idx) => (
                            <button 
                                ...
                                className={`aspect-square rounded-lg overflow-hidden border-2 transition-all ${activeImage === img ? 'border-primary ring-2 ring-primary/30' : 'border-transparent hover:border-slate-300'}`}
                            >
                ...
                <div className="flex flex-col">
                  <div className="flex flex-wrap justify-between gap-3"><p className="text-[#111318] text-4xl font-black leading-tight tracking-[-0.033em] min-w-72">{productTitle}</p></div>
                  <p className="text-slate-600 text-base font-normal leading-normal pb-3 pt-1">{product.subtitle}</p>
                    ...
                    <div className="flex text-primary">
                      {[1, 2, 3, 4, 5].map((star) => {
                        ...
                        if (rating >= star) {
                          ...
                          return <span key={star} className="material-symbols-outlined text-xl text-slate-300">star</span>;
                        }
                      })}
                    </div>
                  ...
                  <h1 className="text-primary text-[22px] font-bold leading-tight tracking-[-0.015em] text-left pb-3 pt-5">{price}</h1>
                  <div className="mt-4">
                    <h3 className="text-sm font-semibold text-slate-700 mb-2">Color: <span className="font-normal">{selectedColor || product.color || 'Not specified'}</span></h3>
                        ...
                        {safeColors.length > 0 ? (
                            safeColors.map((c, idx) => (
                                <div 
                                    ...
                                    className={`w-8 h-8 rounded-full cursor-pointer ring-2 ring-offset-2 ring-offset-background-light border border-slate-200 ${selectedColor === c ? 'ring-primary' : 'ring-transparent'}`}
                            ...
                            <div 
                                className="w-8 h-8 rounded-full ring-2 ring-primary ring-offset-2 ring-offset-background-light border border-slate-200"
                  ...
                  <div className="mt-6">
                    <h3 className="text-sm font-semibold text-slate-700 mb-2">Select Size</h3>
                        ...
                        safeSizes.map((size) => (
                          <button 
                            ...
                            className={`p-3 border rounded-lg text-center text-sm font-medium transition-colors ${
                              ...
                              : 'border-slate-300 hover:border-primary text-slate-800'
                            }`}
                          >
                            {size}
                          </button>
                  ...
                  <div className="mt-8 flex items-center gap-4">
                    <div className="flex items-center border border-slate-300 rounded-lg">
                      <button onClick={() => setQuantity(q => Math.max(1, q - 1))} className="p-3 text-slate-500 hover:text-primary"><span className="material-symbols-outlined text-base">remove</span></button>
                      <span className="px-4 font-semibold text-slate-800">{quantity}</span>
                      <button onClick={() => setQuantity(q => q + 1)} className="p-3 text-slate-500 hover:text-primary"><span className="material-symbols-outlined text-base">add</span></button>
                    </div>
                    <button onClick={handleAddToCart} className="flex-1 text-white bg-primary hover:bg-primary/90 focus:ring-4 focus:ring-primary/30 font-semibold rounded-lg text-base px-6 py-3.5 text-center transition-colors">Add to Cart</button>
                  </div>
                  <button className="mt-4 w-full text-primary bg-primary/10 hover:bg-primary/20 font-semibold rounded-lg text-base px-6 py-3.5 text-center transition-colors">Buy Now</button>
                  <div className="mt-8 space-y-4">
                    <div className="border-b border-slate-200 pb-4">
                      <h4 className="text-base font-semibold text-slate-800">Product Description</h4>
                      <p className="mt-2 text-sm text-slate-600">{product.description || 'No description available.'}</p>
                    </div>
                    <div className="border-b border-slate-200 pb-4">
                      <h4 className="text-base font-semibold text-slate-800">Materials &amp; Care</h4>
                      <p className="mt-2 text-sm text-slate-600">{product.materialsCare || 'No materials information available.'}</p>
                    </div>
                    <div>
                      <h4 className="text-base font-semibold text-slate-800">Shipping &amp; Returns</h4>
                      <p className="mt-2 text-sm text-slate-600">{product.shippingInfo || 'No shipping information available.'}</p>
                    </div>
                  </div>
                </div>
              </div>
